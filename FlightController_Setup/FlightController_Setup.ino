...
void loop(){
  ...
  TWBR = 12;                      //Setting I2C clock speed to 400kHz.
  ...
  if(error == 0){
    ...
    updateReceiverFromRadio();
    Serial.println(" ");
    //Storing the central stick positions
  ...
  if(error == 0){
    ...
    if(whoami == 0x68 || whoami == 0x70){  // Accept clones returning 0x70
  ...
  if(error == 0){
    // Storing all the information in the EEPROM
    Serial.println(F(""));
    Serial.println(F("==================================================="));
    ...
    //Verifying the EEPROM data to make sure evrything is ok,
...
void wait_sticks_zero(){
  ...
  while(zero < 15){
    updateReceiverFromRadio();
    if(receiver_input_channel_1 < center_channel_1 + 50 && receiver_input_channel_1 > center_channel_1 - 50)zero |= 0b00000001;
